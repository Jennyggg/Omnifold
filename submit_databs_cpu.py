...
#SBATCH --time=1-00:00:00                   # time  in format DD-HH:MM
...
cd ${{TMPDIR}}
python mytrain.py -m {method} -u {unfold} -mc {MC} -data {data} {arguments_train} --bootstrap --bsseed {seed} 
rm -rf /scratch/$USER/${{SLURM_JOB_ID}}
"""

for seed in range(int(opts.seed),int(opts.seed)+int(opts.queue)):
  script_name = "train_bs_"+str(seed)+".sh"
