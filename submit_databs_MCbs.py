...
cd ${{TMPDIR}}
python mytrain.py -m {method} -u {unfold} -mc {MC} -data {data} {arguments_train} --bootstrap --bsseed {seed} --MCbootstrap --MCbsseed {seed}
rm -rf /scratch/$USER/${{SLURM_JOB_ID}}
"""

for seed in range(int(opts.seed),int(opts.seed)+int(opts.queue)):
  script_name = "train_coverage_"+str(seed)+".sh"
